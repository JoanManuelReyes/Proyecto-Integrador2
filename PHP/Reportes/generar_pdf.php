...
if ($producto !== 'Todos') {
    $sql .= "  WHERE e.Producto_id = '".mysqli_real_escape_string($conn, $producto)."'";
}
...
if ($producto !== 'Todos') {
    // Extrae solo el número después de "PR"
    $sql .= " WHERE s.Producto_id = '".mysqli_real_escape_string($conn, $producto)."'";
}
...
while ($row = mysqli_fetch_array($result)) {
    $tableRows .= '<tr>';
    $tableRows .= '<td>' . htmlspecialchars($row['MovimientoID']) . '</td>';
    $tableRows .= '<td>' . htmlspecialchars($row['Tipo']) . '</td>';
    $tableRows .= '<td>' . htmlspecialchars($row['Producto']) . '</td>';
